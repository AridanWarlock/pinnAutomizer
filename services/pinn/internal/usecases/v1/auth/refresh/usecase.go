...
import (
	...
	"github.com/AridanWarlock/pinnAutomizer/pinn/internal/usecases/v1/auth"
...
type Postgres interface {
	...
	RotateRefreshToken(ctx context.Context, oldHash string, newHash string, newJti core.Jti) error
}
...
func (u *usecase) Refresh(ctx context.Context, in Input) (Output, error) {
	...
	audit := core.MustAuditInfoFromContext(ctx)
...
func (u *usecase) deleteOldSession(ctx context.Context, jti core.Jti) error {
...
func (u *usecase) setNewRedisSession(
	...
	session, err := core.NewRedisSession(
		userID,
		roles,
		fingerprint,
		issuedAt,
