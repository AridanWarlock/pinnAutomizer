...
func New(
	...
	usecase = &Usecase{
		...
		log: log.With().Str("component", "usecase: task.UpdateTaskStatusAfterTrain").Logger(),
	}

	return usecase
}

func (u *Usecase) UpdateTaskStatusAfterTrain(ctx context.Context, in Input) error {
	...
	err := u.postgres.UpdateTaskStatusByID(ctx, in.ID, string(domain.TaskStatusDone))
	if err != nil {
